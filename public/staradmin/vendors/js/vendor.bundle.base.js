...
!(function (t, e) {
    ...
    class ye extends M {
        ...
        show() {
            // Check if the dropdown is already shown or if the element is not valid
            if (d(this._element) || this._menu.classList.contains("show"))
                return;

            const parentElement = ye.getParentFromElement(this._element);
            const eventDetails = { relatedTarget: this._element };

            // Trigger the 'show.bs.dropdown' event
            if (
                // !I.trigger(this._element, "show.bs.dropdown", eventDetails)
                //     .defaultPrevented
            ) 
            {
                // If the dropdown is in the navbar, we skip the Popper.js positioning
                if (this._inNavbar) {
                    ...
                    // Determine the reference element based on configuration
                    if (this._config.reference === "parent") {
                        referenceElement = parentElement;
                    } else if (r(this._config.reference)) {
                        referenceElement = this._config.reference.jquery
                    ...
                    const applyStylesModifier = popperConfig.modifiers.find(
                        (mod) => mod.name === "applyStyles" && !mod.enabled
                    );

                    // Initialize Popper.js for positioning the dropdown
                    this._popper = ce(
                        referenceElement,
                        this._menu,
                        popperConfig
                ...
                // Focus the element and update aria attributes for accessibility
                this._element.focus();
                this._element.setAttribute("aria-expanded", "true");

                // Show the dropdown menu and the toggling element
                this._menu.classList.toggle("show");
                this._element.classList.toggle("show");

                // Trigger the 'shown.bs.dropdown' event after the dropdown is shown
                I.trigger(this._element, "shown.bs.dropdown", eventDetails);
            }
        }
